...
class PopupHandler {
  ...
  Future<void> startImageAnimation() async {
    ...
    _imageTimer = Timer.periodic(frameDuration, (timer) {
      _currentImageIndex = (_currentImageIndex + 1) %
          (imagePathsByBodyPart[_currentBodyPart]?.length ??
              defaultImagePaths.length);

      _imageNotifier.value = _currentImageIndex; // ✅ 애니메이션 적용
      print(
          "🎞 Updating image index: $_currentImageIndex for $_currentBodyPart"); // 🔥 로그 확인
    });
  }
  ...
  Future<void> triggerAnimation(String bodyPart,
    ...
    _imageTimer = Timer.periodic(frameDuration, (timer) {
      _currentImageIndex =
          (_currentImageIndex + 1) % imagePathsByBodyPart[bodyPart]!.length;
  ...
  // 터치 이벤트 및 팝업
  void showPopupForCoordinates(BuildContext context, Offset tapPosition,
  ...
  // 리소스 해제
  void dispose() {
    stopImageAnimation(); // 캐릭터 애니메이션 중단
    stopPeriodicStatusUpdate(); // 상태 주기적 업데이트 중단
  }
}
