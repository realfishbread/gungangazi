...
class _SignUpPageState extends State<SignUpPage>
  ...
  void _completeSignUp() async {
    ...
    try {
      ...
      // final message = await userRepository.registerUser(user); //map을 스트링처럼 반환하지 마시오

      final result = await userRepository.registerUser(user); // ← Map 형태
  ...
  void _validatePassword(String password) {
    ...
    if (password.isEmpty) {
      ...
      setState(() {
        _passwordFeedback = '비밀번호는 8자 이상, 대문자, 소문자, 숫자, 특수문자를 포함해야 합니다.';
        _isPasswordValid = false;
      });
    } else {
      setState(() {
  ...
  Widget _buildStepForm() {
    return Column(
      children: [
        ...
        const SizedBox(height: 16),
        Text(
            '단계: ${steps[_currentStep]} (${_currentStep + 1}/${steps.length})'),
        TextButton(
