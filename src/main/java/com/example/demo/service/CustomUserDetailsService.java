...
public class CustomUserDetailsService implements UserDetailsService {
    ...
    public UserDetails loadUserByUsername(String loginValue) throws UsernameNotFoundException {
        User user = userRepository.findByUsername(loginValue)
            .or(() -> userRepository.findByEmail(loginValue)) // username 먼저 찾고 없으면 email 검색
            .orElseThrow(() -> new UsernameNotFoundException("User not found with username or email: " + loginValue));
        ...
        return org.springframework.security.core.userdetails.User.builder()
            .username(user.getUsername()) // username 반환 (email 아님)
            .password(password)
            .build();
    }
